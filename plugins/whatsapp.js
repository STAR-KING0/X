const { haki, parsedJid, localBuffer } = require('../utils');
...
haki(
 ...
 async (message, match) => {
  ...
  if (!msg) return await message.reply('_Message not found maybe haki might not be running at that time_');
...
haki(
 {
  pattern: 'edit ?(.*)',
  fromMe: true,
  desc: 'Edit message sent by the haki',
  type: 'whatsapp',
 },
 async (message, match, m, client) => {
...
//     type: 'whatsapp'
//   },
//   async (message,match,m,client,haki) => {
